function reload(Mi, myContainer) {
    var gyerekek = (Mi == '2') ? 'Intézmények' : 'Gyerekek'
    var gyerek = (Mi == '2') ? 'intézmény' : 'gyerek'
    queue()
        .defer(d3.csv, "data/kp" + Mi + ".csv")
        .defer(d3.csv, "data/kl" + Mi + ".csv")
        .defer(d3.json, "data/kq" + Mi + ".json")
        .await(drawAll);
    ...
    function drawAll(error, ageCSV, idCSV, occupations) {
        ...
        var padding = 20,
            width = Math.max($(myContainer).innerWidth(), 350) - padding,
        ...
        var hiddenCanvas = d3.select(myContainer).append("canvas")
            .attr("id", "hiddenCanvas" + Mi)
            .attr("width", width)
            .attr("height", height)
            .style("display", "none");
        ...
        var elementsPerBar = 7,
            barChartHeight = 0.7,
            barChartHeightOffset = 0.35;

        //The draw function of the canvas that gets called on each frame
        function drawCanvas(chosenContext, hidden) {

            //Clear canvas
            chosenContext.fillStyle = "rgba(255, 255, 255, 1)";
            ...
            for (var i = 0; i < nodeCount; i++) {
                ...
                if (node.ID in dataById) {
                    ...
                    if (node.ID.lastIndexOf(currentID, 0) === 0 & !hidden) {
                        ...
                        var drawTitle = true;
                        var fontSizeTitle = Math.round(nodeR / 9);
        ...
        $("#canvas" + Mi).on("click", clickFunction);
        ...
        if (!mobileSize) {
            ...
            $("#canvas" + Mi).on("mousemove", mousemoveFunction);
        ...
        function zoomToCanvas(focusNode) {
            ...
            $("#canvas" + Mi).css("pointer-events", "none");
        ...
        function interpolateFadeText(dt) {
            if (fadeText) {
                ...
                if (timeElapsed >= fadeTextDuration) {
                    ...
                    $("#canvas" + Mi).css("pointer-events", "auto");
        ...
        var select = document.getElementById("searchBox" + Mi);
